-- [[ 🟢 CHRISSHUB V2 SUPREME - V3 FINAL STABLE 🟢 ]]
-- [[ DEVELOPER: SASWARE32 | TIKTOK: @sasware32 ]]-- [[ 🟢 CHRISSHUB V2 SUPREME - V3 FINAL STABLE 🟢 ]]
...
-- [[ 👁️ MOTOR ESP (CORREGIDO: DETECTA JUGADORES NUEVOS, NO BORRA) ]]
local active_esp = {}
...
local function CreateESP(p)
    ...
    connection = RunService.RenderStepped:Connect(function()
        if p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") then
            ...
            -- El ESP solo se apaga si la opción está desactivada o el jugador murió
        ...
        -- SOLO SE BORRA SI EL JUGADOR SE VA DEL JUEGO
...
-- [[ 🏙️ MENÚ AZUL NEÓN (300+ LÍNEAS ESTRUCTURA) ]]
local function BuildMain()
    ...
    tpS.MouseButton1Click:Connect(function()
        ...
        task.wait(3)
        local sh = nil; for _, p in pairs(Players:GetPlayers()) do if GetRole(p) == "Sheriff" and p.Character then sh = p break end end
        if sh then lp.Character.HumanoidRootPart.CFrame = sh.Character.HumanoidRootPart.CFrame * CFrame.new(0,0,2); SendNotify("TELEPORTACIÓN EXITOSA", Color3.new(0, 1, 0))
...
local function RunKeys()
    ...
    btn.MouseButton1Click:Connect(function()
        if table.find(CH_KEYS, input.Text) then
            ...
            task.spawn(function() while btn.Text == "Verifying key..." do btn.TextTransparency = 0.5; task.wait(0.3); btn.TextTransparency = 0; task.wait(0.3) end end)
